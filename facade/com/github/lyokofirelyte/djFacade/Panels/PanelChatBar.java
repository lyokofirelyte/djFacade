...
public class PanelChatBar implements AR, Panel {
	
	public DJFacade main;
	private GUI gui;
	
	public PanelChatBar(DJFacade i){
		main = i;
	}
	...
	public void display(){
		...
		try {
			...
			gui.addAttr(new HintTextField(""), "chat");
			
			gui.getPanel("main").setLayout(new FlowLayout());
			gui.panel().setBackground(new Color(0, 0, 0, 0.5f));
			gui.getHintTextField("chat").setPreferredSize(new Dimension(mainGui.getWidth()-10, 25));
			gui.hintTextField().setActionCommand(ActionCommand.CHAT_BAR.toString());
			gui.hintTextField().addActionListener(main.getEventListener());
			
			gui.getPanel("main").add(gui.getHintTextField("chat"));
