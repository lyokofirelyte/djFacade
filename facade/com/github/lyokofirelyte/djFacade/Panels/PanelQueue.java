...
public class PanelQueue implements AR, Panel {
	...
	public void display(){
		...
		try {
			...
			List<JSONObject> queue = new ArrayList<JSONObject>(main.getQueue());
			update(queue, map);

			gui.getPanel("main").setLayout(new FlowLayout());
			gui.panel().setBackground(new Color(0, 0, 0, 0.0f));
			
			gui.add(gui.getScroll("main_scroll"));
			gui.setLocation(mainGui.getX() + 5, (mainGui.getY() - mainGui.getHeight()) - 35);
	...
	public void update(List<JSONObject> queue, JSONMap map){
